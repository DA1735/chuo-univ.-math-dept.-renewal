...
const Colloquium: React.FC = () => {
  const [colloquiumData, setColloquiumData] = useState<UnifiedEvent[]>([]);
  const [loading, setLoading] = useState(true);

  React.useEffect(() => {
    fetchSeminars('colloquium')
      .then((data: any[]) => {
        const parsed = data.map(item => ({
          ...item,
          documents: typeof item.documents === 'string' ? JSON.parse(item.documents) : item.documents,
        }));
        setColloquiumData(parsed);
        setLoading(false);
      })
      .catch(err => {
        console.error("Failed to fetch colloquium:", err);
        setLoading(false);
      });
  }, []);

  const { displayUpcoming, sortedPastEvents } = useMemo(() => {
    if (colloquiumData.length === 0) return { displayUpcoming: null, sortedPastEvents: [] };
    const allEvents = [...colloquiumData].sort((a, b) => b.number - a.number);
        ...
        <div className="container mx-auto max-w-5xl">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <p className="text-gray-300 text-lg max-w-2xl leading-relaxed">
              数学科の全教員・大学院生を対象とした、最も中心的なセミナーです。<br />
              専門分野の枠を超え、数学の多様な側面に触れる機会を提供します。
            </p>
          </motion.div>
        </div>
      </section>
            ...
            {displayUpcoming && (
              <div className="mb-24">
                <div className="flex items-center gap-3 mb-8">
                  ...
                  <h2 className="text-2xl font-bold text-gray-800">最新の開催記録</h2>
                </div>
                <UnifiedEventCard
